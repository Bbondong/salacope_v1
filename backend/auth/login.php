<?php
// Configuration InfinityFree compatible
error_reporting(0);
...
try {
    // 1. Vérifier dans la table admin (Num seulement)
    $query = "SELECT *
              FROM admin 
              WHERE Num = :username 
              LIMIT 1";
    ...
    // 2. Vérifier dans la table client (tel seulement)
    $query = "SELECT *
              FROM client 
              WHERE tel = :username 
              LIMIT 1";
    ...
    if ($stmt->rowCount() > 0) {
        ...
        if ($passwordValid) {
            // Migration du mot de passe si en clair
            if ($isPlainText) {
